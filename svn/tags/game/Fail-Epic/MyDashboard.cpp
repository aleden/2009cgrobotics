...
int run_dashboard(...)
{
	const float dashboardDelay = 0.2;
	for (;;) {
		dashboardDataFormat->updateDashboard();
		dashboardDataFormat->PackAndSend();
		Wait(dashboardDelay);
	}
}

void DashboardDataFormat::updateDashboard()
{
	for (UINT32 i = 0; i < kPwmChannels; i++)
		m_PWMChannels[0][i] = hal->getByPWMRaw(i);
}
